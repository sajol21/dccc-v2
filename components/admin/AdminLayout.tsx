...
const AdminLayout: React.FC<AdminLayoutProps> = ({ user, signOut, children }) => {
    const navGroups = [
        ...
        { 
            name: 'Content', 
            tabs: [
                { name: 'Home Page', path: '/admin/content/home' },
                { name: 'About Page', path: '/admin/content/about' },
                { name: 'Departments', path: '/admin/content/departments' },
                { name: 'Events', path: '/admin/content/events' },
                { name: 'Achievements', path: '/admin/content/achievements' },
            ] 
        },
        {
            name: 'Team',
            tabs: [
                { name: 'Manage Panels', path: '/admin/team/panels' },
         ...
         { 
            ...
            tabs: [
                ...
                { name: 'Footer', path: '/admin/settings/footer' },
            ] 
        },
    ];
    ...
    const { activeGroupName } = useMemo(() => {
        for (const group of navGroups) {
             if (group.isSingle && location.pathname.startsWith(group.path)) {
                return { activeGroupName: group.name };
            }
            if (group.tabs) {
                const isActive = group.tabs.some(tab => location.pathname.startsWith(tab.path));
                if (isActive) {
                    return { activeGroupName: group.name };
                }
            }
        }
        return { activeGroupName: 'Dashboard' };
