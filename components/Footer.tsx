            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-6">
                <div className="flex flex-col items-center text-center">
                    ...
                    <div className="mb-4 text-sm">
                        ...
                        <a href={`mailto:${footerData.email}`} className="text-blue-600 hover:underline">{footerData.email}</a>
                        {' | '}
                        <a href={`tel:${footerData.phone.replace(/\s/g, '')}`} className="text-blue-600 hover:underline">{footerData.phone}</a>
                    </div>

                    {/* Social Links */}
                    <div className="flex justify-center space-x-6 mb-4">
                        {footerData.socialLinks.map(social => (
                            <a key={social.name} href={social.url} target="_blank" rel="noopener noreferrer" className="text-gray-600 hover:text-blue-600 transition-colors" aria-label={social.name}>
